...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if (message.poll.selectableCount < 0 || message.poll.selectableCount > message.poll.values.length) {
            throw new boom_1.Boom(`poll.selectableCount in poll should be between 0 and ${message.poll.values.length} or equal to the items length`, { statusCode: 400 });
        }
        const keyy = node_crypto_1.default.randomBytes(32);
        m.messageContextInfo = {
            messageSecret: keyy
        };
        m.pollCreationMessage = Types_1.WAProto.Message.PollCreationMessage.fromObject({
            name: message.poll.name,
            selectableOptionsCount: message.poll.selectableCount,
            options: message.poll.values.map(value => Types_1.WAProto.Message.PollCreationMessage.Option.fromObject({
                optionName: value,
            }))
        });
    }
